...
from pyspark.sql.functions import from_json, to_json, col, lit, unix_timestamp, current_timestamp
...
def foreach_batch_function(df, epoch_id):
    ...
    df.write.jdbc(
        ...
        mode='append',
        properties={'user': 'jovyan', 'password': 'jovyan'}
    )
    ...
    kafka_df.selectExpr("CAST(restaurant_id AS STRING) AS key", "to_json(struct(*)) AS value") \
        .write \
        .format("kafka") \
        .option("kafka.bootstrap.servers", "rc1b-2erh7b35n4j4v869.mdb.yandexcloud.net:9091") \
        .option("topic", "aksenov_nikita_out") \
        .save()
